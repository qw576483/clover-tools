...
func (wb *WorkBook) parseBof(buf io.ReadSeeker, b *bof, pre *bof, offset_pre int) (after *bof, after_using *bof, offset int) {
	...
	case 0x809:
		bif := new(biffHeader)
		_ = binary.Read(buf_item, binary.LittleEndian, bif)
		if bif.Ver != 0x600 {
			wb.Is5ver = true
		}
		wb.Type = bif.Type
	case 0x042: // CODEPAGE
		_ = binary.Read(buf_item, binary.LittleEndian, &wb.Codepage)
		case 0x3c: // CONTINUE
		if pre.Id == 0xfc {
			var size uint16
			var err error
	...
	case 0xfc: // SST
		info := new(SstInfo)
		_ = binary.Read(buf_item, binary.LittleEndian, info)
	...
	case 0x85: // bOUNDSHEET
		...
		_ = binary.Read(buf_item, binary.LittleEndian, bs)
		// different for BIFF5 and BIFF8
		wb.addSheet(bs, buf_item)
	case 0x0e0: // XF
		if wb.Is5ver {
			xf := new(Xf5)
			_ = binary.Read(buf_item, binary.LittleEndian, xf)
			wb.addXf(xf)
		} else {
			xf := new(Xf8)
			_ = binary.Read(buf_item, binary.LittleEndian, xf)
	...
	case 0x031: // FONT
		f := new(FontInfo)
		_ = binary.Read(buf_item, binary.LittleEndian, f)
		wb.addFont(f, buf_item)
	case 0x41E: //FORMAT
		font := new(Format)
		_ = binary.Read(buf_item, binary.LittleEndian, &font.Head)
		font.str, _ = wb.get_string(buf_item, font.Head.Size)
		wb.addFormat(font)
	case 0x22: //DATEMODE
		_ = binary.Read(buf_item, binary.LittleEndian, &wb.dateMode)
		}
	return
}
...
func (w *WorkBook) get_string(buf io.ReadSeeker, size uint16) (res string, err error) {
	if w.Is5ver {
		...
		if flag&0x1 != 0 {
			...
			var n int
			n, err = buf.Read(bts)
			// #nosec G115 -- n 从长度 size（uint16）的缓冲区读取，不会溢出。
			if uint16(n) < size {
				// #nosec G115 -- 同上，n <= size。
...
func (w *WorkBook) prepareSheet(sheet *WorkSheet) {
	_, _ = w.rs.Seek(int64(sheet.bs.Filepos), 0)
	sheet.parse(w.rs)
}
				...
				for k, row := range sheet.rows {
					...
					if len(row.cols) > 0 {
						for _, col := range row.cols {
							// #nosec G115 -- Excel 列索引最大 65535，data 长度不会溢出。
